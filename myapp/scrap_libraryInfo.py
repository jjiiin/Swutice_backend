...
dept_kr='문헌정보학과'
deptNum=10
def scraping_libraryInfo():
    url="http://swulis.net/?pageid=1&page_id=51&mod=list"
    headers={
    ...
    for index, value in enumerate(announcements):
        ...
        if temp_index not in "공지사항":
            title_=str(temp[1].find("div",{"class":"kboard-default-cut-strings"}).text).replace("\n","").replace("N","").replace(" ","")
            title=str(temp[1].find("div",{"class":"kboard-default-cut-strings"}).text).replace("\n","").replace("N","")
            ...
            if title_before != title_:
                ...
                link="http://swulis.net"+find_link
...
def contentExtraction(link):
    ...
    result=""
    try:
        content_text=soup.find("div",{"class":"content-view"}).text.replace("\n","")
